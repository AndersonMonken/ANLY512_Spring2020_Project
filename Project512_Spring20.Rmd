...
plot_testerrorrandom <- ggplot(data = decision_df, aes(x = ntree, y = test_mse, color = as.factor(mtry))) + 
  ...
  theme_bw() +
  theme(legend.title = element_blank()) + ylim(60,100)


plot_testerrorrandom_zoom <- ggplot(data = decision_df %>% filter(ntree > 10 & ntree < 150), aes(x = ntree, y = test_mse, color = as.factor(mtry))) + 
  geom_line(size = 1.1) +
  ylab("Test MSE") + 
  xlab("Number of Trees") + 
...
gbm_plot <- ggplot(data = decision_df_gbm %>% mutate(interaction.depth = as.factor(interaction.depth)), aes( x = ntree, y = test_mse, color =  interaction.depth)) + 
  geom_line() + xlab("Number of Trees") + ylab("Test MSE") +
...
```{r}
gbm_plot_zoom <- ggplot(data = decision_df_gbm %>% mutate(interaction.depth = as.factor(interaction.depth)) %>% filter(shrinkage == 0.1), aes( x = ntree, y = test_mse, color =  interaction.depth)) + 
  geom_line() + xlab("Number of Trees") + ylab("Test MSE") +
  ggtitle("GBM Shrinkage=0.1") + scale_color_manual(values = my_colors) + ylim(70,100) + 
  geom_point(data = data.frame(y = 76.21, x = 1500), aes(x = x, y = y), color = "red", size = 5, shape = 2)
...
gbm_importance <- ggplot(summary_gbm_importance, aes(x = reorder(var, rel.inf),y=rel.inf)) + geom_col(fill = 'darkblue') + coord_flip() + ylab("Relative Importance") + xlab("Variable") + ggtitle("Variable Importance in Best GBM Model")
