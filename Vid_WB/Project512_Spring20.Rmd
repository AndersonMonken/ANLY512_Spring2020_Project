...
library(tree)
# Decision Trees: 
tree.train = tree(Max_Partners ~ GDP_per_unit_CO2 + Govt_Revenue + gdp_per_cap +agri_perc_gdp + mobilesub_per100peeps+ intl_tourist_arrival + total_life_exp, data = train)

summary(tree.train)

plot(tree.train)
text(tree.train, prett = 0)



# Now we use the cv.tree() function to see if pruning the tree will improve performance. 
...
# Using pruned tree model to get the MSE

yhat.ptree = predict(prune.train, newdata = test)
plot(yhat.ptree,test$Max_Partners)
abline(0,1)
mse_ptree <- mean((yhat.ptree-test$Max_Partners)^2)
# Bagging Method:

bag.train = randomForest(Max_Partners ~ GDP_per_unit_CO2 + Govt_Revenue + gdp_per_cap +agri_perc_gdp + mobilesub_per100peeps+ intl_tourist_arrival + total_life_exp, data = train,mtry = 7, ntree = 25,importance = TRUE)

bag.train

yhat.bag = predict(bag.train, newdata = test)
plot(yhat.bag, test$Max_Partners)
abline(0,1)
mse_bag <- mean((yhat.bag - test$Max_Partners)^2)
mse_bag

# Random Forest: 

rf.train = randomForest(Max_Partners ~ GDP_per_unit_CO2 + Govt_Revenue + gdp_per_cap +agri_perc_gdp + mobilesub_per100peeps+ intl_tourist_arrival + total_life_exp, data = train, mtry = 2, ntree = 25,importance = TRUE)

yhat.rf = predict(rf.train, newdata = test)
mse_rf <- mean((yhat.rf - test$Max_Partners) ^ 2)
mse_rf
...
# Modeling: Non-linear Regression
```{r}
nonlinear_fit <- lm(Max_Partners ~ mobilesub_per100peeps+ log(intl_tourist_arrival) + log(total_life_exp), data = train)

yhat.nonlinear <- predict(nonlinear_fit,test)

mse_nonlinear <- mean((yhat.nonlinear - test$Max_Partners)^2)
mse_nonlinear

huxreg(nonlinear_fit) # cool formatting
```


# Modeling: SUMMARY- All Methods:
```{r}
# test MSE for every method: 
test_MSE
mse_nonlinear
mse_ptree
mse_bag
...
MSE = c(test_MSE,mse_nonlinear,mse_ptree,mse_bag,mse_rf)
Model = c('Linear Regression',"Non-Linear Regression","Decision Tree Regression","Bagging", "Random Forest")
